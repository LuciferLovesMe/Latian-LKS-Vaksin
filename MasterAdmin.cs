...
namespace LKS_Vaksin
{
    public partial class MasterAdmin : Form
    {
        ...
        void loadgrid()
        {
            string com = "select * from admin where level = 1";
        ...
        {
            string com = "select * from admin nama where nama like '%" + textboxsearch.Text + "%' and level = 1 ";
            dataGridView1.DataSource = Command.getdata(com);
            dataGridView1.Columns[2].Visible = false;
            dataGridView1.Columns[4].Visible = false;
        }
        ...
        private void btnhapus_Click(object sender, EventArgs e)
        {
            if (dataGridView1.CurrentRow.Selected)
            {
                ...
                {
                    DialogResult result = MessageBox.Show("Apakah anda yakin ingin menonaktifkan?", "Konfirmasi", MessageBoxButtons.OK, MessageBoxIcon.Question);
                    if(result == DialogResult.Yes)
                    {
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            if(cond == 1 && val())
            {
                ...
                string com = "insert into admin values('" + textBox2.Text + "', '" + pass + "', 1, 1, '" + textBox1.Text + "')";
                try
                {
                    Command.exec(com);
        ...
        {
            MasterVaksin master = new MasterVaksin();
            this.Hide();
            master.ShowDialog();
        }
